...
raw %>% 
  group_by(byear) %>% 
  summarize(mean_death_age = mean(death_age)) %>% 
  arrange(desc(mean_death_age))

# 1860 and 1854 have the highest averages because those people 
# have had the chance to live the longest. The year with the lowest
# average will be 1940.
```
...
# January, December, March and February have the highest deaths.
# global warming is causing harsher winters.
```
...
x <- raw %>% 
  ...
  mutate(death_date = as.Date(paste(dyear, dmonth, 01, sep = "."), format = "%Y.%m.%d")) %>% 
...
x <- raw %>%  
  mutate(season = ifelse(dmonth < 4 | dmonth > 9, 'Cold', 'Warm')) %>% 
  mutate(death_date = as.Date(paste(dyear, dmonth, 01, sep = "."), format = "%Y.%m.%d")) %>% 
  select(death_date, death_age, season) %>% 
